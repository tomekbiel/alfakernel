...
extern int RESPONSE_PORT = 5565;
extern int COMMAND_PORT = 5566;
extern int SUB_PORT = 5557;
extern int MILLISECOND_TIMER = 5;
extern int MILLISECOND_TIMER_PRICES = 750;

extern string t0 = "--- Trading Parameters ---";
extern int MaximumOrders = 1;
extern double MaximumLotSize = 0.1;
...
extern bool Publish_MarketRates = true;
...
string Publish_Symbols[13]={"US.100+","EURUSD+", "OIL.WTI+", "OILs+", "GOLDs+",  "US.500+", 
							"US.30+", "DE.30+", "JAP225+","VIX+", "W.20+", "EURPLN+", "USDJPY+"};
string Publish_Symbols_LastTick[13]={"US.100+","EURUSD+", "OIL.WTI+", "OILs+", "GOLDs+",  "US.500+", 
							"US.30+", "DE.30+", "JAP225+","VIX+", "W.20+", "EURPLN+", "USDJPY+"};
...
int OnInit() {
   ...
   // Send responses to COMMAND_PORT that client is listening on.   
   if(!pushSocket.bind(StringFormat("%s://%s:%d", ZEROMQ_PROTOCOL, HOSTNAME, COMMAND_PORT))) {
      Print("[PUSH] ####ERROR#### Binding MT4 Server to Socket on Port " + IntegerToString(COMMAND_PORT) + "..");
      return(INIT_FAILED);
   } else {
      Print("[PUSH] Binding MT4 Server to Socket on Port " + IntegerToString(COMMAND_PORT) + "..");
      pushSocket.setSendHighWaterMark(1);
      pushSocket.setLinger(0);
   }
   
   // Receive commands from RESPONSE_PORT that client is sending to.     
   if(!pullSocket.bind(StringFormat("%s://%s:%d", ZEROMQ_PROTOCOL, HOSTNAME, RESPONSE_PORT))) {
      Print("[PULL] ####ERROR#### Binding MT4 Server to Socket on Port " + IntegerToString(RESPONSE_PORT) + "..");
      return(INIT_FAILED);
   } else {
      Print("[PULL] Binding MT4 Server to Socket on Port " + IntegerToString(RESPONSE_PORT) + "..");
      pullSocket.setReceiveHighWaterMark(1);   
      pullSocket.setLinger(0); 
   }
   
   // Send new market data to SUB_PORT that client is subscribed to.      
   if(!pubSocket.bind(StringFormat("%s://%s:%d", ZEROMQ_PROTOCOL, HOSTNAME, SUB_PORT))) {
      Print("[PUB] ####ERROR#### Binding MT4 Server to Socket on Port " + IntegerToString(SUB_PORT) + "..");
      return(INIT_FAILED);
   } else {
      Print("[PUB] Binding MT4 Server to Socket on Port " + IntegerToString(SUB_PORT) + "..");
...
void OnDeinit(const int reason) {
   ...
   Print("[PUSH] Unbinding MT4 Server from Socket on Port " + IntegerToString(COMMAND_PORT) + "..");
   pushSocket.unbind(StringFormat("%s://%s:%d", ZEROMQ_PROTOCOL, HOSTNAME, COMMAND_PORT));
   pushSocket.disconnect(StringFormat("%s://%s:%d", ZEROMQ_PROTOCOL, HOSTNAME, COMMAND_PORT));
   
   Print("[PULL] Unbinding MT4 Server from Socket on Port " + IntegerToString(RESPONSE_PORT) + "..");
   pullSocket.unbind(StringFormat("%s://%s:%d", ZEROMQ_PROTOCOL, HOSTNAME, RESPONSE_PORT));
   pullSocket.disconnect(StringFormat("%s://%s:%d", ZEROMQ_PROTOCOL, HOSTNAME, RESPONSE_PORT));
   
   if (Publish_MarketData == true || Publish_MarketRates == true) {
      Print("[PUB] Unbinding MT4 Server from Socket on Port " + IntegerToString(SUB_PORT) + "..");
      pubSocket.unbind(StringFormat("%s://%s:%d", ZEROMQ_PROTOCOL, HOSTNAME, SUB_PORT));
      pubSocket.disconnect(StringFormat("%s://%s:%d", ZEROMQ_PROTOCOL, HOSTNAME, SUB_PORT));
   }
...
void InterpretZmqMessage(Socket &pSocket, string &compArray[]) {
   ...
   // e.g. TRADE|OPEN|1|US.100+|0|50|50|R-to-MetaTrader4|12345678
   ...
   // 3.2) TRACK_RATES|INSTRUMENT_1|INSTRUMENT_2|...|INSTRUMENT_N  -> List of instruments to receive OHLC rates
           ...
           //       Symbol: US.100+, Timeframe: PERIOD_M1 ----> Instrument = "US.100+_M1"          
   ...
   /*
      ...
      If TRADE ->
         ...
         compArray[3] = Symbol (e.g. US.100+, etc.)
   ...
   */
    ...
    ****************************/
      ...
      case 2: // MODIFY TRADE
         ...
         ans = DWX_ModifyOrder(StrToInteger(compArray[10]), StrToDouble(compArray[4]),
                               StrToDouble(compArray[5]), StrToDouble(compArray[6]), 3, zmq_ret);
         InformPullClient(pSocket, zmq_ret + "}");
         break;
      ...
      case 7: // GET OPEN TRADES
         zmq_ret = "{";
         DWX_GetOpenOrders(zmq_ret);
         InformPullClient(pSocket, zmq_ret + "}");
         break;

      case 8: // HISTORICAL DATA REQUEST
         zmq_ret = "{";
         DWX_GetHist(compArray, zmq_ret);
         InformPullClient(pSocket, zmq_ret + "}");
         break;

      case 9: // TRACK PRICES (for LIVE feed)
         zmq_ret = "{";
         DWX_SetSymbolList(compArray, zmq_ret);
         InformPullClient(pSocket, zmq_ret + "}");
         break;

      case 10: // TRACK RATES (for candle feed)
         zmq_ret = "{";
      ...
      default:
         InformPullClient(pSocket, "{'_error': 'Nieobsługiwane polecenie'}");
         break;
   }
...
void DWX_GetHist(string& compArray[], string& zmq_ret) {
   ...
   zmq_ret = zmq_ret + "}";  // zamknięcie JSON-a
   InformPullClient(pushSocket, zmq_ret);  // wysłanie do Pythona
